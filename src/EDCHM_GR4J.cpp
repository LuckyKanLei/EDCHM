...
{
  NumericVector param_percola_gr4_k(n_spat);

  int n_UH_land = ceil(max(X_4)), n_UH_ground = ceil(max(X_4));
  NumericVector P_n, Q_r, E_s, P_s, Perc_, Pr_1, Pr_9,
  Q_1(n_spat), Q_9(n_spat), F_, Q_d, F_9, P_, E_, E_n, A_E;
  ...
  for (int j= 0; j < n_spat; j++) {
    UH_2(_, j) = confluenIUH_GR4J2(X_4(j));
    UH_1(_, j) = confluenIUH_GR4J1(X_4(j));
  ...
  for (int i= 0; i < n_time; i++) {
    ...
    Pr_1 = 0.1 * (P_n - P_s + Perc_); //
    Pr_9 = 0.9 * (P_n - P_s + Perc_);


    for (int ii= 0; ii < n_UH_land - 1; ii++) {
      mat_Pr_1(ii + 1,_) = mat_Pr_1(ii, _);

    }
    mat_Pr_1(0, _) = Pr_1;

    for (int ii= 0; ii < n_UH_ground - 1; ii++) {
      mat_Pr_9(ii + 1,_) = mat_Pr_9(ii, _);
